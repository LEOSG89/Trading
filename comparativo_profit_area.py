...
def mostrar_profit_area(df: pd.DataFrame, chart_key: str) -> None:
    ...
    excl = st.multiselect(
        "Excluir índices Profit Tot.:", df_idx,
        default=excl, key=f"ms_{chart_key}_profit"
    )
    try:
        with open(json_file, 'w') as f:
            json.dump(excl, f)
    ...
    profit_plot_num = pd.to_numeric(
        profit_plot_str.str.rstrip('%'), errors='coerce'
    )
    profit_tbl_str = (
        df['Profit T.'].astype(str)
        if 'Profit T.' in df.columns else profit_plot_str
    )
    ...
    x_vals = plot_idx
    y_vals = profit_plot_num.loc[plot_idx].fillna(0)
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=text_vals,
            hovertemplate='Índice: %{x}<br>Profit Tot.: %{text}',
            name='Profit Tot.'
        )
    )
