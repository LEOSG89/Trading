...
def summary_all_years(df: pd.DataFrame) -> pd.DataFrame | None:
    ...
    # Quitar depósito inicial de la suma del primer mes
    if (first_year, first_month) in summary.index:
        summary.at[(first_year, first_month), 'Depósitos'] -= base_amount
...
def calculate_daily(df: pd.DataFrame, year: int, month: int, asset: str, tipo: str) -> tuple[pd.DataFrame, float]:
    ...
    if year == dep_year and month == dep_month:
        ...
        prev_profit = df[
            (df['Fecha'] >= first_dep['Fecha']) &
            (df['Fecha'] < first_of_month) &
            (df['Deposito'] == 0)
        ]['Profit'].sum()
...
def mostrar_calendario(df_raw: pd.DataFrame, chart_key: str = "calendario") -> None:
    ...
    with tabs[0]:
        ...
        if not years:
            ...
            return
        default_year = saved.get('year', years[-1])
    ...
    with tabs[3]:
        ...
        default_month = saved.get('month', months[0] if months else None)
        if default_month not in months:
            default_month = months[0] if months else None
        month = st.selectbox("Selecciona Mes", months,
    ...
    with tabs[4]:
        ...
        default_asset = saved.get('asset', assets[0] if assets else "")
        if default_asset not in assets:
            default_asset = assets[0] if assets else ""
        asset = st.selectbox("Selecciona Activo", assets,
    ...
    with tabs[5]:
        ...
        if default_tipo not in tipos:
            default_tipo = 'AMBAS'
        tipo = st.selectbox("Selecciona Tipo Op", tipos,
                            index=tipos.index(default_tipo),
