...
def mostrar_tiempo_puntos(df: pd.DataFrame, chart_key: str) -> None:
    ...
    tiempo_fmt = {i: f"{tiempo_min[i]//1440}d {((tiempo_min[i]%1440)//60):02d}:{(tiempo_min[i]%60):02d}" for i in df_idx}
    ...
    with tabs[2]:
        excl_points = st.multiselect("Excluir puntos manualmente:", df_idx, default=excl_points, key=f"pts_{chart_key}")
    ...
    def get_max_bin(mask):
        counts, _ = np.histogram([tiempo_min[i] for i in df_idx if mask(i)], bins=edges)
    ...
    default_green  = (edges[get_max_bin(mask_pos)],  edges[get_max_bin(mask_pos) + 1])
    default_red    = (edges[get_max_bin(mask_neg)],  edges[get_max_bin(mask_neg) + 1])
    default_yellow = (edges[get_max_bin(mask_zero)], edges[get_max_bin(mask_zero) + 1])

    # 11) Slider Franjas
    def create_slider(tab, label, key, default_range):
        with tab:
            opts = [f"{int(e//1440)}d {int((e%1440)//60):02d}:{int(e%60):02d}" for e in edges]
            default = tuple(excl_data.get(key, (
                f"{int(default_range[0]//1440)}d {int((default_range[0]%1440)//60):02d}:{int(default_range[0]%60):02d}",
                f"{int(default_range[1]//1440)}d {int((default_range[1]%1440)//60):02d}:{int(default_range[1]%60):02d}"
            )))
            return st.select_slider(label, options=opts, value=default, key=f"slider_{key}_{chart_key}")
    ...
    # Profit positivos/ceros/negativos
    ...
    excl_data.update({
        ...
        'green': selected_green, 'red': selected_red, 'yellow': selected_yellow
    })
